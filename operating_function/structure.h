...
struct Courses{
    ...
    Course* getCourse(string courseID, int session_x, int day_x) {
        Course* cur = monitor;
        while (cur->nextCourse != NULL) {
            if ((cur->nextCourse->courseID) == courseID && ((session_x == cur->session[0] && day_x == cur->day[0]) || (session_x == cur->session[1] && day_x == cur->day[1]) )) {
                return cur->nextCourse;
            }
            cur = cur->nextCourse;
        }
